...
- **Bot de clientes**: agregar **@vrf2fa_bot** al grupo `-1005538610567` y desactivar **Group Privacy** en @BotFather (`/setprivacy` → @vrf2fa_bot → Disable); definir `TELEGRAM_CLIENTES_BOT_TOKEN` y `TELEGRAM_CLIENTES_CHAT_ID=-1005538610567` en las variables del servicio de Railway (en el `.env` local ya están); probar `/start`, los botones y `/nombre` (admin) desde el grupo — las 15 cuentas ya vienen cargadas por defecto en `data/clientes_bot.json`
