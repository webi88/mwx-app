...
- **Bot de clientes**: agregar **@vrf2fa_bot** al grupo `-1005538610567` (Telegram → grupo → Agregar miembros → buscar el bot) y desactivar **Group Privacy** en @BotFather (`/setprivacy` → @vrf2fa_bot → Disable); definir `TELEGRAM_CLIENTES_BOT_TOKEN` en las **variables del servicio de Railway** (en el `.env` local ya está; el bot corre dentro de supervisord, ya NO hay servicio `bot-clientes` aparte), registrar clientes (`/start` en el grupo una vez + `/asignar <telegram_id> [nombre] <usuarios...>`) y probar con 1-2 cuentas (el ID del dueño quedó como "Prueba (dueño)" con las 15 cuentas en `data/clientes_bot.json` local)
