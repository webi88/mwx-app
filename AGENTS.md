...
- **Bot de clientes**: crear el bot con @BotFather y poner `TELEGRAM_CLIENTES_BOT_TOKEN` (local/Railway), agregar el servicio `bot-clientes` (docker-compose ya listo), registrar clientes (`/start` del cliente + `/asignar <telegram_id> [nombre] <usuarios...>` del admin) y probar con 1-2 cuentas
