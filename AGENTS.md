...
- **Bot de clientes**: agregar **@vrf2fa_bot** al grupo `-1005538610567` (Telegram → grupo → Agregar miembros → buscar el bot) y desactivar **Group Privacy** en @BotFather (`/setprivacy` → @vrf2fa_bot → Disable); poner `TELEGRAM_CLIENTES_BOT_TOKEN` en el servicio `bot-clientes` (docker-compose ya listo; en el `.env` local ya está), registrar clientes (`/start` en el grupo una vez + `/asignar <telegram_id> [nombre] <usuarios...>`) y probar con 1-2 cuentas (el ID del dueño quedó como "Prueba (dueño)" con las 15 cuentas en `data/clientes_bot.json` local)
- **App passwords de Yahoo**: generar "contraseña de aplicación" para cada correo de las 15 cuentas de cliente y re-importarla (`importar_clientes.py --apply` solo actualiza `email_password`); sin eso el código por correo del bot de clientes no funciona (Yahoo rechazó la password normal por IMAP)
