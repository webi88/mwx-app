...
### Sistema de Cuotas Inteligente por Hora: 500+ acciones/h seguras (2026-09-21)
- **Objetivo**: cuentas NO premium; maximizar volumen sin disparar el anti-spam de X repartiendo la carga por cuenta y por rol.
- `core/config.py`: `LIMITE_POSTS_HORA=5` (rol hashtags), `LIMITE_CITAS_HORA=5` (cita), `LIMITE_RTS_HORA=7` (rt), `LIMITE_COMENTARIOS_HORA=3` (comentario), `LIMITE_VENTANA_MIN=60` y `CUOTAS_HORARIAS_ACTIVO=1` (0 = sin cuotas). Documentadas en `.env.example`. ⚠ Los alias por `env=` de `Field` son IGNORADOS por pydantic-settings 2.5.2 (los `LIMITE_*` funcionan por coincidir con el nombre del campo); `limite_cuotas_activo` usa `validation_alias="CUOTAS_HORARIAS_ACTIVO"`.
- `core/registro.py`: `ACCIONES_POR_ROL` (roles -> tipos ya registrados: post/mantenimiento/calentamiento/hilo, cita/quote, rt/retweet/repost, comentario/respuesta), `normalizar_rol_cuota`, `tipo_registro_rol`, `limite_por_rol` y `contar_acciones_recientes(usuario, rol, minutos=60)` (un solo COUNT de exitosas en RegistroAccion; nunca lanza) + `contar_acciones_por_usuario(usuarios, minutos)` (una consulta agrupada para la base del contador).
- `activaciones/cuotas.py` (nuevo): `CuotasHorarias` thread-safe; `usado = base_bd + exitos_campana + reservas_en_vuelo`; `reservar` y `liberar` atomicos bajo `RLock` (la reserva se toma ANTES de credenciales/retardo/navegador y se cierra en `finally`: con exito consume, sin exito devuelve), `viables`, `agotado`, `avisar_agotada` (throttle 1/min), `uso`, `resumen`.
- `activaciones/motor.py`:
  - Seleccion: `_asignar_roles_aleatorios(..., cuotas=None)` quita de las opciones los roles agotados y deja "" (sin accion) a la cuenta sin ningun cupo; en roles fijos `_generar_textos_ronda` filtra los grupos sin mutarlos. Con `cuotas=None` el comportamiento es identico al anterior.
  - Ejecucion: `_ejecutar_accion_rol` y `_quote_rt_una_cuenta` reservan cupo atomico; sin cupo devuelven `ok=None` + "cuota agotada..." SIN abrir navegador ni registrar en BD. Los reportes tratan `ok=None` como OMITIDA (claves nuevas `omitidas_por_cuota` y `cuotas` en los resumenes; `omitidas` en `snapshot_progreso`).
  - Log debug exacto (1/min/cuenta): `Cuenta X alcanzó todos sus límites por hora, descansando`.
  - Registro por rol: las acciones de campana ahora se guardan con `tipo_registro_rol(rol)` (post/cita/rt/comentario) para que el conteo funcione ENTRE campanas; antes todo era "activacion".
- **Capacidad**: 20 acciones/hora/cuenta (5+5+7+3); con 25 cuentas activas = hasta 500/h. `CUOTAS_HORARIAS_ACTIVO=0` desactiva todo.
- Verificado: `compileall` global OK; `tests/run_tests.py` **660/660** (75 core + 60 cuotas nuevos; suite previa 516); carrera de 30 hilos con limite 5 = exactamente 5 exitos; E2E con reloj falso: cuenta con hashtags agotado publica solo cita/rt, cuenta con comentarios agotados sigue con posts/rt, cuenta con TODO agotado = 0 ejecuciones y 8 omisiones; `CUOTAS_HORARIAS_ACTIVO=0` -> False/0/ilimitado y `=1` -> True/7; mensaje debug exacto y throttle verificados.
- ⚠ **Redeploy en Railway** para que apliquen las cuotas; ajustar los `LIMITE_*` por env segun la edad/reputacion de la flota (subirlos solo con cuentas maduras).

### Limite estricto de 100 caracteres en posts y RTs con cita (2026-09-22)
- `ia/prompts.py`: constante publica `REGLA_MAX_100` (texto exacto del dueño) inyectada en `_reglas_registro(registro, incluir_limite=True)` (default True, tambien sin registro) y de forma explicita en `get_prompt_hashtags` y `_prompt_reposteo`; `_reglas_formato` default 100; comentario (200) y blog (600) quedan FUERA con `incluir_limite=False`.
- `ia/generador_contenido.py`: `_MAX_LARGO_HASHTAG = 100`; helper `_acotar_limite(texto, limite=100)` (detecta hashtags y delega en `_recortar_limite_hashtag`: corte en cierre de frase o ultimo espacio, hashtag EN MEDIO, nunca al final, sin palabras a medias) aplicado en citas/variaciones, `generar_pool_por_cuenta`, posts de mantenimiento (comentarios intactos), citas de campana, 3+3+3 y `generar_contenido` (solo tipos post; blog/verificado/harfuch conservan su limite). El recorte CONSERVA duplicados: contrato exacto de cantidad.
- Verificado: `compileall` global OK; suite **716/716**; fuzz de 300 casos sin `len>100` ni palabras a medias; IA simulada de 367 chars -> 67; prompts sin "240".
- ⚠ `_recortar_para_x` de `plataformas/twitter/selenium_bot.py` sigue en 280 (red externa de X); comentarios fuera del limite.

### Tabla de exitos con fila vacia entre registros + descarga Excel (2026-09-22)
- `web/operaciones/reportes.py`: `_filas_con_separacion(filas)` (exito, vacia, exito, ..., vacia, exito; idempotente; sin vacias con 0-1 exitos) se usa en el `st.dataframe` del historial de acciones (y por tanto en el CSV del dataframe); `_excel_exitos_bytes(filas)` genera el .xlsx EN MEMORIA (openpyxl, cabeceras con estilo 1DA1F2) y la pagina agrega el boton "⬇️ Descargar Excel (.xlsx)" (key `dl_reportes_exitos`).
- Verificado: `compileall web tests` OK; suite **744/744** (28 checks nuevos en `tests/test_reportes_excel.py`); xlsx real: fila 2 exito / fila 3 vacia / fila 4 exito.

### Reparto de roles por porcentajes en Activacion Masiva (2026-09-22)
- `web/operaciones/activacion_masiva.py`: nuevo `_repartir_por_porcentajes(usuarios, pesos=None)` (resto mayor; suma exacta = n; bloques contiguos conservando el orden; limpia '@'/vacios/duplicados; roles con peso 0 nunca reciben; normaliza sumas != 100; default 25/25/25/25). La pestana "🗂️ Por roles" muestra el bloque "🎚️ Reparto por porcentajes" (4 inputs 0-100, preview "Con N cuentas seleccionadas: ...", boton deshabilitado si la suma != 100) que asigna `Cuenta.rol_activacion` a las cuentas seleccionadas (p. ej. selector Filtro -> Seccion: Libertad).
- **Eliminados** (a pedido del dueño, para no "mezclar" roles automaticamente): botones "🎯 Reparto trending (45/25/20/10)", "🎯 Preset: Trending 1 hora", "🎲 Repartir automaticamente entre los 4 roles", checkbox "🎲 Rol aleatorio por cuenta en cada ronda" y helpers (`_repartir_trending`, `PESOS_TRENDING`, `_repartir_tercios`, `_aplicar_preset_trending`, `PRESET_TRENDING`). La campana usa `roles_aleatorios=False` (rol fijo); `activaciones/motor.py` conserva el soporte aleatorio sin cambios.
- Verificado: suite **750/750** (11 checks nuevos + AppTest de la pestana con 0 excepciones); reparto real de 25 cuentas con `{cita:20, hashtags:40, comentario:0, rt:40}` -> 5/10/0/10.

### Eliminacion masiva de cuentas por lista + respaldo (2026-09-22)
- `cuentas/eliminador.py` (nuevo): `parsear_lista_usuarios(texto)` (una cuenta por linea; acepta `usuario`, `@usuario` o la linea completa del vendedor `usuario:password:totp:...`; ignora vacias y comentarios `#`; dedupe case-insensitive conservando la primera forma), `buscar_cuentas(usuarios, plataforma="twitter")` (coincidencia case-insensitive contra `Cuenta.usuario`; devuelve `encontradas/no_encontradas/error`; nunca lanza) y `eliminar_usuarios(usuarios, borrar_cookies=True, cancelar_tareas=True, respaldar=True)` (respaldo JSON en `data/backups/eliminacion_cuentas_*.json` ANTES del DELETE — si el respaldo falla no borra nada; una sola transaccion; borra `data/cookies/{plataforma}/{usuario}.pkl` y avatar/portada solo si estan dentro de `data/`; cancela tareas `pendiente` cuyos `cuentas_ids` pertenezcan TODOS a cuentas eliminadas; devuelve `eliminadas/no_encontradas/archivos_borrados/tareas_canceladas/respaldo/error`).
- `eliminar_cuentas.py` (nuevo, raiz): CLI `python eliminar_cuentas.py [lista.txt] [--usuarios "a,b"] [--apply] [--sin-cookies] [--sin-tareas] [--sin-respaldo] [--json]`; dry-run por defecto; nunca imprime credenciales. Para Supabase, exportar `DATABASE_URL` de Railway antes de correr (igual que `restaurar_cuentas.py`).
- `web/operaciones/cuentas.py`: nueva pestana **"🗑️ Eliminar"** (modo "🗂️ Cuentas", entre Estado e Inventario): se pega la lista, "🔍 Buscar coincidencias" la cruza contra la BD y **autoselecciona todas las encontradas** (multiselect ajustable), muestra encontradas/no encontradas, opciones de respaldo/cookies/tareas (ON por defecto) y boton "🗑️ Eliminar definitivamente (N)" con confirmacion; tras borrar limpia cache y muestra resumen con la ruta del respaldo.
...
### Tiers + Curva de Aceleración + cuota diaria global (2026-09-22)
- **Cuota diaria global (pedido del dueño)**: `LIMITE_DIARIO_POR_CUENTA` (alias legado `LIMITE_ACCIONES_DIA`; `AliasChoices`) default **12** acciones OPERATIVAS exitosas por cuenta en 24 h (`LIMITE_DIA_VENTANA_MIN=1440`, interruptor `CUOTAS_DIARIAS_ACTIVO`). `core/registro.py`: `ACCIONES_OPERATIVAS` (14 tipos de los 4 roles), `limite_acciones_dia()`, `contar_acciones_dia`, `contar_acciones_dia_por_usuario`, `esta_agotada_dia`, `usuarios_agotados_dia` (1 consulta agrupada). "Agotada por hoy" = alcanzó el tope; el motor la descansa y, si hay respaldo, la sustituye.
- **Tiers de calidad**: `core/tiers.py` (nuevo: `normalizar_tier`, `etiqueta_tier`, `tier_de_cuenta`, `es_tier2`, `rol_permitido_tier`, `error_rol_tier`, `ROL_PROHIBIDO_TIER2="hashtags"`), `Cuenta.tier_calidad` + migración automática + export `Tier_Calidad`. OJO: `post`/`mantenimiento`/`hilo` normalizan a `hashtags`, así que Tier 2 también tiene prohibidos esos alias.
- **Blindaje Tier 2 ⇔ hashtags**: en `activaciones/motor.py` el modo fijo excluye a los Tier 2 con rol efectivo hashtags (`tier2_hashtags_omitidas` + usuarios, sin abrir navegador) y el modo aleatorio les quita `hashtags` de sus opciones (`tier2_sin_rol`); `solo_roles=["hashtags"]` también los excluye. En la UI: pestaña "🏅 Tiers" y el reparto por porcentajes/lanzamiento de Por roles bloquean con `st.error` y "Sistema no modificado" (no escriben nada).
- **Curva de Aceleración ("Explosión de Tendencia")**: `curva_aceleracion=True` + `curva_fase1_min` (env `CURVA_FASE1_MIN`, default 15) en `ejecutar`/`ejecutar_por_roles`. Fase 1 (0-15 min): SOLO Tier 1 elegible (gate `elegibilidad` evaluado al iniciar cada ronda en `_bucle_rondas`; sin elegibles la ronda no avanza ni genera textos y los workers esperan). Fase 2: se liberan Tier 2 (y sin tier) para RT/citas/comentarios **en cascada sobre las URLs publicadas por los Tier 1** (`_capturar_url_fase1`, cap 50, fallback a las URLs de campaña; `cascada_urls`). `progreso["fase_actual"]` visible en la UI ("🚀 Fase N/2"). Sin Tier 1 o `duracion_min <= fase1` se desactiva/ajusta con WARNING. `repetir=False` hace 2 etapas secuenciales.
- **Cuentas de respaldo**: `reserva_usuarios=[...]` (usuarios o Cuentas; `RESERVA_MAX_CUENTAS` default 50) precargadas con sesión; al iniciar cada ronda las cuentas "agotadas por hoy" se retiran y se sustituyen 1:1 por una reserva del MISMO rol (una reserva Tier 2 jamás sustituye hashtags); contadores `rotadas_por_cuota_dia`, `reserva_usada`, `reserva_disponible`, `agotadas_dia`. La UI las elige (activas+sesión, misma sección, sin la selección principal).
- **Hilos/RAM**: `PESTANA_MAX_MINUTOS` (default 20) recicla la pestaña persistente por EDAD además de por acciones; `MAX_WORKERS` acotado a [1, 32]; al terminar cada campaña se loguea `acciones/min` y la RAM máxima del proceso (módulo `resource`, si existe).
- **Cuotas diarias en `CuotasHorarias`**: `limite_dia/minutos_dia/activo_dia`, `agotado_dia`, `uso_dia`, `resumen()["diarias"]`; reservas atómicas horarias Y diarias (sin pasar el tope en paralelo). `CUOTAS_DIARIAS_ACTIVO=0`/límite 0 = comportamiento clásico exacto.
- **Blindaje del compositor de X**: `_abrir_compositor` agrega una fase final de recuperación (refresh corto + reintento de `/compose/post`) para interstitiales "Something went wrong"; `_pegar_texto` re-destruye la capa `[data-testid="mask"]` y reescribe UNA vez si el overlay bloqueó; `publicar_tweet` hace UN ciclo limpio silencioso (refresh + recomponer) ante fallo transitorio. Nada de `logger.error` en transitorios manejados; contratos de mensajes intactos (citas/respuestas solo usan el reintento de máscara).
- **Validación de hashtags**: `ia/generador_contenido.validar_texto_hashtag(texto, hashtags, limite=100)` (todos los pedidos, ninguno ajeno, en ventana 15-85%, no al inicio/fin, no pegado a artículo, ≤100) y `garantizar_texto_hashtag(...)` aplicado como paso FINAL en `generar_textos_hashtags_por_cuenta` (IA y fallback). Fuzz 220 casos: 0 violaciones.
- **Dashboard**: inventario con badge `🏅 Tier 1 / 🧱 Tier 2` y columna "Cuota hoy" (`N/12 · 🔴 Agotada por hoy`, cache 60 s); pestaña "🏅 Tiers"; checkbox "🚀 Estrategia: Curva de Aceleración" (fase 1 configurable) y expander "🛡️ Cuotas diarias y respaldo" en las 2 pestañas de Activación Masiva; métricas de curva/reserva/agotadas en resultados.
- Verificado: `compileall` global OK; `tests/run_tests.py` **1352/1352** (nuevos: `test_cuotas_diarias_core` 212, `test_cuotas_diarias_motor` 32, `test_tiers_motor` 26, `test_curva_motor` 45, `test_reserva_diaria_motor` 41, `test_hashtags_validacion` 47, `test_ui_tiers_curva` 42, `test_selenium_fakes` 101); smoke del coordinador: cuota diaria bloquea el 4º rol y devuelve cupo al fallar, Tier 2 bloqueado en post/mantenimiento, firmas del motor con los kwargs nuevos, `LIMITE_DIARIO_POR_CUENTA=7`→7 y `CUOTAS_DIARIAS_ACTIVO=0`→0.
- ⚠ **Railway debe redeployarse** para aplicar todo esto. Config recomendada: `LIMITE_DIARIO_POR_CUENTA=12`, `CUOTAS_DIARIAS_ACTIVO=1`, `PESTANA_MAX_MINUTOS=20`, `RESERVA_MAX_CUENTAS=50`, `CURVA_FASE1_MIN=15`. Antes de una campaña con curva: asignar Tiers (Tier 1 = líderes con hashtags; Tier 2 = volumen para rt/citas/comentarios).

### Paro total inmediato + proceso persistente tras refresh (2026-09-23)
- **Problema**: no habia boton de paro: el hilo seguia hasta agotar `duracion_min` y el guard de campana unica bloqueaba relanzar durante minutos; la vista del proceso vivia solo en el script run que lanzaba, asi que al navegar/recargar desaparecia (solo quedaba Reportes).
- **Motor - cancelacion**: `ejecutar`/`ejecutar_por_roles` aceptan `cancelar=None` (threading.Event, AL FINAL de la firma); `solicitar_paro()` interno (fallback) + `_cancelado()`/`_dormir_cancelable()`; chequeos en `_bucle_rondas` (inicio/worker/gate), caminos de una pasada, `_ejecutar_accion_rol`, `_quote_rt_una_cuenta`, `_adquirir_navegador`/`_adquirir_pestana`, `_esperar_turno_comentario` y todos los sleeps largos (tramos <=0.5s). Las acciones no ejecutadas devuelven `ok=None` + `MENSAJE_CANCELADO` (cuentan como omitidas: sin BD ni navegador). Resumen `cancelada` SIEMPRE + `omitidas_por_cancelacion`; `snapshot_progreso()["cancelada"]`. Salida en segundos sin accion en vuelo; una accion Selenium en vuelo termina con su timeout fail-fast (~20-45s).
- **Dashboard - Paro total**: boton `⛔ Paro total` en el panel en vivo (`web/operaciones/activacion_masiva.py`); si el hilo sigue vivo a los `PARO_FORZAR_SEG=90s` aparece `⚠️ Forzar liberación` (manual, avisa que la campana vieja puede seguir cerrando Chrome).
- **Dashboard - registro persistente**: `_CAMPANAS` a nivel modulo (motor/hilo/evento/estado/parametros/resumen) + guard con TOKEN (`_CAMPANA_ID`/`_liberar_campana(id)`: el hilo viejo jamas libera el guard de una campana nueva); JSON atomico `data/campanas/ultima_campana.json` (saneado: URLs sin query, sin motor/hilo/evento/previos_env; carpeta en `.gitignore`). Panel `_render_proceso_activo` al inicio de las 2 pestanas con `st.fragment(run_every=1s)` (fallback bucle `sleep(1)+st.rerun()`); al recargar/navegar el proceso se repinta desde el registro o, si ya murio, desde el JSON ("ultimo proceso").
- **Envs de velocidad**: el restore (`previos_env`) ahora ocurre en el `finally` del hilo runner: un refresh a mitad de campana ya no deja `CHROME_SIN_IMAGENES`/`API_PRIMERO`/etc. aplicados para siempre.
- Verificado: `tests/run_tests.py` **1436/1436** (`test_cancelacion_motor` 41 + `test_ui_paro_progreso` 43); compileall global OK; smoke del coordinador: paro pre-seteado = 0 acciones/`cancelada=True`/`_dormir_cancelable(600)` en 0.00s, registro+estado+paro, JSON saneado y atomico, guard con token (el hilo viejo no pisa al nuevo).
- ⚠ **Railway debe redeployarse**. Relanzar tras un paro es inmediato; "Forzar liberación" solo si un hilo queda atascado >90s (puede solapar el cierre de Chrome de la campana vieja).

### Links reales en la tabla de exitos (2026-09-23)
- **Motor** (`activaciones/motor.py`): helper `_url_publicada_valida(url, usuario)` (solo links con `/status/`, limpia query/fragmento; JAMAS perfiles ni el tweet ancla) y `_capturar_url_post()` (env `CAPTURAR_URL_POST`, default 1). `RegistroAccion.url_publicacion` por rol: cita -> post de la cita; hashtags/post -> post publicado (`publicar_tweet(buscar_url=_capturar_url_post())`); comentario -> URL de la respuesta devuelta por `responder_tweet`; **rt simple y like -> `""` siempre**. Aplica a las rutas clasica, pestana persistente (`_quote_rt_en_pestana`/`_ejecutar_accion_en_pestana`) y API (rt/like `""`; cita/comentario/hashtags usan `TwitterAPI.ultima_url_publicada`).
- **Selenium** (`plataformas/twitter/selenium_bot.py`): nuevo `_obtener_enlace_reciente_timeline(texto_publicado, timeout=3)` (ruta RAPIDA sin navegar: primer articulo del timeline actual de la PROPIA cuenta que contenga el texto publicado; acepta el handle real) usado por `publicar_tweet(buscar_url=True)` y la rama de cita de `solo_retwittear`, con fallback al perfil (`_obtener_ultimo_enlace`, ahora con espera explicita de articulos en vez del `sleep(4)` fijo); `responder_tweet` deja `ultima_url_publicada`; el RT simple ya NO devuelve el perfil en `urls`.
- **API** (`plataformas/twitter/api_http.py`): `TwitterAPI.ultima_url_publicada` expone `https://x.com/<usuario>/status/<id>` cuando GraphQL devuelve el id del tweet creado (contrato bool de `accion_rapida` intacto).
- **Reportes** (`web/operaciones/reportes.py`): tabla de exitos con columnas `Fecha, Usuario, Tipo, Resultado, Link` (`st.column_config.LinkColumn` clicable; las filas vacias de separacion siguen). `_resultado_accion(tipo, url)` etiqueta: cita/post/comentario con link o "(link no capturado)"; rt/like "(no genera link)" con `""` SIEMPRE (aunque haya URL historica de perfil o del tweet likeado). `_es_url_post(url)` exige `/status/`. Excel con las 5 columnas; eliminado el bloque markdown duplicado de links.
- Verificado: `tests/run_tests.py` **1515/1515** (`test_urls_exitos_motor` 39, `test_reportes_excel` 51, `test_selenium_fakes` 118); compileall global OK; smoke del coordinador (validador, URLs por rol con fakes incluido el ancla descartada, y clasificacion Resultado/Link de Reportes).
- ⚠ Trade-off: `CAPTURAR_URL_POST=1` (default) paga ~1-3s por post en la ruta rapida (hasta ~5-10s si cae al perfil) para tener el link; `CAPTURAR_URL_POST=0` lo desactiva (la tabla mostrara "link no capturado"). La API solo aporta link si X devuelve el id del tweet creado.

### Tier 3 (Métricas/Soporte): blindaje total + curva en cascada (2026-09-23)
- **Tercer nivel** (`core/tiers.py`): `TIERS["tier3"] = "Tier 3 (Métricas / Soporte)"`; `normalizar_tier` acepta "tier3"/"3"/"granja"/"farm"/"metricas"/"soporte"/"baja calidad" y la etiqueta completa; nuevas `ROLES_PROHIBIDOS_TIER3 = ("hashtags","cita","comentario")` y `ROLES_PERMITIDOS_TIER3 = ("rt","like")`; `es_tier3(cuenta)`; `rol_permitido_tier` estricto (Tier 3 → True SOLO con rol efectivo rt/like o vacío; Tier 2 sigue prohibido solo hashtags); `error_rol_tier` con mensaje propio de Tier 3 (el de Tier 2 quedó byte a byte). `core/models.py`: solo el comentario del campo (String(20), sin migración).
- **Motor** (`activaciones/motor.py`): el modo fijo descarta Tier 3 de los grupos hashtags/cita/comentario antes de abrir navegador (`tier3_omitidas`/`tier3_omitidas_usuarios`; Tier 2 conserva `tier2_hashtags_*`); el modo aleatorio filtra por `_tier_permitido` y enruta "sin rol" por tier (`tier3_sin_rol`/`tier3_sin_rol_usuarios` vía `_contar_sin_rol_por_tier`); defensa en `_obtener_cuentas_por_rol` (`_tier3_filtradas_rol`); **Cita masiva** (`ejecutar`) excluye Tier 3 de cuentas y de reservas (todas ejecutan "cita"); **Curva de Aceleración**: fase 1 solo Tier 1 (Tier 2/Tier 3 completamente ignorados: sin textos ni navegador), fase 2 libera Tier 3 solo en rt/like y en cascada a las URLs de los Tier 1 (`_urls_efectivas_rol` suma "like"), contador `tier3_liberadas`; **Reservas**: `_tier_compatible_reserva` — una Tier 3 agotada por la cuota diaria solo la sustituyen reservas Tier 2 o Tier 3 (NUNCA Tier 1 ni sin tier), contador `sustituciones_bloqueadas_tier`.
- **Scheduler**: `scheduler/ejecutor.py` valida `_bloqueada_por_tier` ANTES de crear navegador (Tier 3 + tarea post/comentario/cita → fallo con motivo "tier 3 ... PROHIBIDA", sin reintento y sin Chrome); `scheduler/calentamiento.py` ya no elige Tier 3 (publicaría un post). Cubre Calendario, Reparto por hora y Calentamiento.
- **Dashboard**: `web/operaciones/cuentas.py` — badge `📊 Tier 3 (Métricas / Soporte)` en el inventario, pestaña Tiers con 4 métricas/botones y bloqueo de asignación si alguna seleccionada tiene rol hashtags/cita/comentario (`_errores_roles_tier`); `activacion_masiva.py` — `_bloqueo_roles_tier` (Tier 2 + hashtags; Tier 3 + los 3 roles) en el reparto por porcentajes (valida TODOS los bloques), la asignación manual y el lanzamiento, más métricas de tier3; `_helpers.py::bloqueo_tier_ejecucion` valida el tier antes de crear bot en `ejecutar_en_cuentas` (las omitidas no abren Chrome ni registran), tipos corregidos (`rtc` → "cita", likes → "like"); guards en `posts.py` (hilo/pool) y `reparto_hora.py` (Tier 3 solo RTs en el 3+3+3).
- Verificado: `compileall` global OK; `tests/run_tests.py` **1730/1730** (`test_web_tier3_bloqueo.py` 38, `test_calentamiento.py` 171, `test_tiers_motor.py` 57, `test_curva_motor.py` 59, `test_reserva_diaria_motor.py` 64, `test_ui_tiers_curva.py` 61, `test_cuotas_diarias_core.py` 275); smoke del coordinador (reglas de core.tiers, `_tier_compatible_reserva`, `bloqueo_tier_ejecucion`, guard del scheduler y badge).
- ⚠ **Railway debe redeployarse**. Antes de asignar Tier 3: la cuenta no debe tener rol hashtags/cita/comentario (la UI lo bloquea; el motor y el scheduler lo ignoran igual). En una campaña con curva, fase 1 solo Tier 1 y fase 2 con Tier 3 en cascada (rt/likes).

### Diversificacion masiva del mantenimiento (2026-09-23)
- **Temas**: `ia/prompts.py` pasa de 4 a **42 temas** de mantenimiento (`TEMAS_MANTENIMIENTO`), con instruccion propia en `_INSTRUCCIONES_TEMA` para cada uno (trafico, clima, comida/antojos, series, lunes, insomnio, futbol, memes, motivacion, mascotas, cafe, musica, vecinos, transporte, filas, calor, lluvia, finde, deportes, recuerdos, escuela, trabajo, home office, super, cocina, postres, tacos, netflix, videojuegos, podcast, libros, viajes, playa, pueblo, familia, abuelos, amistad, ejercicio, sueno, tecnologia, celular, apagones, precios...). `_normalizar_tema` con `unicodedata` + ~90 alias/plurales; los 4 legacy conservan su texto. `ia/generador_contenido.py` deriva `_TEMAS_MANTENIMIENTO` de `prompts.TEMAS_MANTENIMIENTO` (una sola fuente).
- **Prompt**: `_prompt_lote_mantenimiento` agrega la **REGLA DE DIVERSIDAD (OBLIGATORIA)** (prohibido repetir tema/estructura/palabras clave/aperturas/cierres/hashtags en el lote; "si un texto habla de cafe, el siguiente del clima") y asigna un **angulo narrativo** rotativo por PERFIL (anecdota, pregunta, opinion, recomendacion, recuerdo, humor, lista, comparacion). Se elimino la agrupacion por tema de los lotes: cada lote de <=15 slots va en orden natural y lleva temas TODOS distintos (indices consecutivos mod 42).
- **Fallback local por partes**: `_PLANTILLAS_MANTENIMIENTO` reescrito como pools (38 sujetos x 27 acciones x 38 comentarios por perfil formal/ciudadano/popular/generico + comentarios) con `_PIEZAS_FAMILIA` (14 familias tematicas) y `_generar_plantilla_local(perfil, tema, semilla, comentario, evitar)` determinista, theme-aware y con evitacion de palabras clave; `_plantilla_local`/`_fallback_estructura_mantenimiento` acumulan `evitar` por cuenta y `vistos_global`; `_plantillas_por_perfil`/`_plantillas_comentario` quedan como wrappers.
- Verificado: `compileall` global OK; suite **1853/1853** con `tests/test_mantenimiento_diversidad.py` (70 checks) + smoke del coordinador: 30 cuentas x 2 con IA caida = 60/60 unicos, 0 cuentas con palabras clave compartidas, posts <=100 con # en medio, comentarios sin #/@/http, 161 palabras tematicas distintas.
- ⚠ **Redeploy en Railway**; probar 1 lote con OpenAI real para calibrar tono (no probado aun con IA real).

### Identidades: proteccion anti-sobrescritura + cruce Registro/Seccion (2026-09-23)
- `cuentas/generador_identidades.py`: nueva `es_handle_generico(handle)` (en `__all__`): detecta basura de proveedor (`Katiaforbx7m`, `GoodWinsnvn`, `khawajaGjdgi`, `Sajtiagosal21`, `qwrtyps12x`) por score combinado (clusters de consonantes invalidos, ratio de vocales <34%, digitos embebidos, transiciones de mayuscula) con whitelist/rescates para no proteger de mas (`UnidosMovCDMX`, `VozLibertad`, `MariaLopez`... dan False). `_cuenta_protegida(cuenta)` protege por `handle_actual` humano, `nombre_mostrado` humano o propuesta previa.
- `asignar_propuestas(..., proteger_brandeadas=False)` (kwarg nuevo AL FINAL): con True OMITE antes de generar (ni IA ni local, no toca la BD) y reporta `omitidas_protegidas`, `protegidas_usuarios` y `protegidas_detalle`; `total` sigue siendo el de la lista recibida. Los lotes ahora se agrupan por **(tipo, seccion, registro)** leyendo `Cuenta.tipo_cuenta` + `Cuenta.seccion`; con registro **ciudadana la seccion se IGNORA** (grupo con `seccion=""`).
- `ia/prompts.py`: nueva `get_prompt_identidades_contexto(seccion, registro, tipo, contexto)` con las reglas: **IP+politica** → formal/corporativo/analista (`@AnalisisIP`, `@ConsultoriaDatos`); **LIB/JUS+activista** → seudonimos combativos/de causa (`@VozLibertad`, `@JusticiaYa`); **ciudadana** → ignora seccion, personas mexicanas reales coloquiales; defaults para otros cruces y tipos (partido conserva colores/simbolos). `_construir_prompt` la inyecta con fallback local.
- `web/operaciones/cuentas.py` (pestaña "🏷️ Nombres"): checkbox **"🛡️ Proteger cuentas ya brandeadas"** (default ON), `_llamar_asignar_propuestas` pasa/omite `proteger_brandeadas` segun firma (fallback TypeError), aviso + expander con las omitidas al generar y **banner persistente en el paso 3** (resumen antes de aplicar) via `st.session_state["nom_protegidas"]`.
- Verificado: `compileall` global OK; suite **1853/1853** (`test_identidades.py` 128/128 con 39 checks nuevos; `test_dashboard_navegacion.py` con AppTest del checkbox/banner/fallbacks) + smoke del coordinador: 19/19 casos `es_handle_generico`, 3/3 omitidas (handle/nombre/propuesta), grupos (movimiento, IP, politica)/(persona, LIB, activista)/(persona, "", ciudadana) y prompt con los ejemplos obligatorios.
- ⚠ **Redeploy en Railway**; al generar nombres en la pestaña Nombres revisar el resumen de omitidas antes de aplicar.

### Comando /codigo: 2FA (TOTP) en vivo desde el bot (2026-09-23)
- `bot/handlers/verificaciones.py` (nuevo): `comando_codigo` (PTB v21 async, `@solo_autorizados`) para `/codigo usuario`; acepta `@usuario` (limpia arrobas/espacios), busca la cuenta con `get_db_session()` (case-insensitive por `Cuenta.usuario`) y genera el código con `pyotp.TOTP(secreto).now()` desde `Cuenta.totp_secret` (columna real; no existe `Cuenta.totp`; el secreto se limpia de espacios y NUNCA se muestra). Respuesta Markdown `🔐 Código 2FA para @handle\n\nNNNNNN\n\n_(Cambia en Ns)_` con segundos restantes reales; muestra `handle_actual` si existe. Errores descriptivos: sin args → uso, cuenta inexistente, sin `totp_secret`, secreto inválido y fallo de BD (nunca sin respuesta).
- `bot/main.py`: registra `CommandHandler("codigo", comando_codigo)` (docstring y log de arranque actualizados).
- Dependencia: `pyotp==2.9.0` ya estaba en `requirements.txt` (se usaba en `plataformas/twitter/selenium_bot.py`); solo se instaló en el venv local.
- Verificado: `compileall` global OK; suite **1853/1853**; agente bot 15/15 (incluye cuenta real con TOTP de la BD y cuenta temporal con secreto con espacios) + coordinador 9/9 (el código coincide con `pyotp.TOTP(secreto).now()`, errores, Markdown, `handle_actual`).
- ⚠ Para usarlo en el grupo: el bot necesita `TELEGRAM_BOT_TOKEN` real y cada miembro que ejecute `/codigo` debe estar en `TELEGRAM_ADMIN_IDS` (el middleware bloquea al resto); la semilla 2FA debe estar importada en `Cuenta.totp_secret`.

### Bot de clientes + cuentas pausadas para activación + fix real de la curva Tier 1 (2026-09-24)
- **Bot de clientes** (`bot_clientes/` NUEVO, PTB v21, token `TELEGRAM_CLIENTES_BOT_TOKEN`, `python -m bot_clientes.main`): bot INDEPENDIENTE del interno, 100% botones para clientes no técnicos. Menú: 🔑 código de verificación (TOTP de `totp_secret` o código del correo vía IMAP), ✏️ cambiar nombre, 📸 foto de perfil, 🖼️ portada, 📋 mis cuentas, ❓ ayuda; solo `/start /ayuda /cancelar` + admin `/clientes /asignar /quitar`. Registro en `data/clientes_bot.json` (Telegram ID → nombre + usuarios; sin credenciales; ignorado en git). Selenium reutiliza `TwitterBot` (`cambiar_nombre`, `cambiar_foto_perfil`, `cambiar_foto_portada`) vía `asyncio.to_thread` con `bot.cerrar()` en `finally`. `docker-compose.yml` servicio `bot-clientes`; `.env.example` y `README.md` actualizados; `tests/test_bot_clientes.py` 92 checks.
- **Lector de código por correo** (`utils/lector_correo.py`, stdlib): `obtener_codigo_verificacion(email, password, timeout=25) -> {ok, codigo, remitente, asunto, fecha, error}` (IMAP SSL Yahoo; INBOX + Bulk/Spam; prioriza remitentes X/Twitter; nunca loguea la contraseña; contrato congelado que consume `bot_clientes/handlers.py`). `tests/test_lector_correo.py` 64 checks. ⚠ **Hallazgo real**: Yahoo RECHAZÓ el login IMAP de las cuentas de cliente (`[AUTHENTICATIONFAILED]`): requieren **contraseña de aplicación** por cuenta (o usar los códigos TOTP); hasta entonces el bot dará TOTP si existe o el error claro del correo.
- **Pausa para activación** (`Cuenta.pausada_activacion` Boolean + `core/pausas.py`: `esta_pausada`/`filtrar_para_activacion`/`pausadas_usuarios`; migración `BOOLEAN DEFAULT FALSE` en SQLite/PostgreSQL; export `Pausada_Activacion`): las cuentas de cliente quedan FUERA de todos los flujos de activación (motor con `pausadas_omitidas`/`pausadas_usuarios` en todos los resúmenes; scheduler bloquea tipos rt/cita/comentario/like con "⏸️ @usuario — pausada para activación (solo mantenimiento)" sin abrir Chrome; selectores de Activación Masiva/RT/Likes/Reparto filtrados + caption; pestaña "⏸️ Estado" con pausar/quitar masivo; columna "Pausa" en inventario; guard `ejecutar_en_cuentas(..., incluir_pausadas=False)`) y SIGUEN en mantenimiento (mantenimiento programado/publicar/IA con `incluir_pausadas=True`; calentamiento continuo las considera elegibles). Tests: `test_pausas.py` 54, `test_pausas_motor.py` 55, `test_web_pausas.py` 26, `test_importar_clientes.py` 39.
- **15 cuentas de cliente importadas** (5 con historial+seguidores, 10 con historial sin seguidores): `importar_clientes.py [archivo] [--apply] [--json]` (UPSERT por usuario; no pisa cookies/totp/sección/tier; fuerza `pausada_activacion=True`; default `data/clientes/a_importar.txt`, `.gitignore` con `data/clientes/`, `*.db-shm`, `*.db-wal`). Aplicado a la BD local: **15/15 creadas y pausadas** con email+email_password. Para Supabase: exportar `DATABASE_URL` de Railway y correr `importar_clientes.py --apply`. ⚠ No traen cookies/sesión ni TOTP: para que el BOT cambie nombre/fotos necesita sesión válida en la BD (hoy no la tienen).
- **Curva Tier 1 — causa raíz y fix** (`activaciones/motor.py`): el gate de fase 1 elegía Tier 1 sin mirar cupos y, si no había utilizables, la fase 1 quedaba MUERTA hasta `curva_fase1_min` (0 acciones: "los Tier 1 se detienen") con el aviso de cuota en `logger.debug`. Ahora `_elegibilidad_curva` considera cupos (`LIMITE_*_HORA`/cooldown/cuota diaria) y si NINGÚN Tier 1 es utilizable **anticipa la fase 2** (WARNING con motivo+usuarios; `curva_fase2_anticipada`/`curva_fase2_motivo` en resumen y en `data/campanas/ultima_campana.json`); lookup de tiers tolerante a espacios/mayúsculas (`_tier_registrado`). `tests/test_curva_tier1_fix.py` 43 checks. Causa operativa frecuente: Tier 1 con `LIMITE_POSTS_HORA=5` agotado o cooldown — subir cuotas, usar más Tier 1 o reservas **Tier 1** para una fase 1 real.
- `bot_clientes/main.py`: guard anti-token-duplicado: si el token de clientes es igual a `TELEGRAM_BOT_TOKEN` NO arranca (Telegram no permite dos procesos con el mismo token: conflicto de polling) y da las instrucciones de @BotFather; `tests/test_bot_clientes.py` 100/100.
- Verificado: `compileall` global OK; suite **2226/2226**; smoke del coordinador (15/15 en BD con pausa, `filtrar_para_activacion=0/15`, `importar_clientes.py --json` OK, `bot_clientes.main` sin token sale con error claro de @BotFather).

### Bot de clientes: uso en GRUPO + tokens separados (2026-09-24)
- **Tokens aclarados**: el bot INTERNO es **@enigmawebi_bot** (`TELEGRAM_BOT_TOKEN`, 8243...) y el bot DE CLIENTES es **@vrf2fa_bot** (`TELEGRAM_CLIENTES_BOT_TOKEN`, 8817...); el `.env` local quedó con ambos. El guard anti-token-duplicado de `bot_clientes/main.py` se conserva (si alguien los reutiliza, el bot de clientes no arranca y explica cómo crear uno nuevo).
- **Soporte de grupo** en `bot_clientes/`: `run_polling(allowed_updates=[..., "my_chat_member"])`; bienvenida única por chat al agregar el bot (`NEW_CHAT_MEMBERS` + `MY_CHAT_MEMBER`); respuestas con mención `👤 @fulano, ...` en grupos (privado byte-idéntico a antes); estado por usuario (PTB `user_data`) y callbacks validan a quien pulsa; el texto libre se captura solo con flujo pendiente (mensaje suelto o reply al mensaje del bot) y el texto de otros miembros se ignora; `/start /ayuda /cancelar` y admin funcionan en grupo. `README.md` documenta desactivar **Group Privacy** en @BotFather (`/setprivacy` → @vrf2fa_bot → Disable) para que los clientes puedan escribir en el grupo. `tests/test_bot_clientes.py` 122 checks.
- El grupo de cliente `-1005538610567` AÚN no tiene al bot: `getChat` con ambos tokens da 400 (no es miembro). Al agregar @vrf2fa_bot mandará la bienvenida solo.

### Actividad: modo leve de campaña con TODOS los hashtags (2026-09-24)
- **Pedido**: además de la activación masiva, un modo LEVE "Actividad": **3-4 tweets por cuenta**, publicaciones suaves y **TODOS los hashtags** (antes los posts de hashtags elegían un subconjunto aleatorio por texto).
- `activaciones/motor.py`: nuevo `MotorActivacion.ejecutar_actividad(usuarios=None, hashtags=None, posts_min=3, posts_max=4, pausa_entre_posts_seg=(60,240), texto_base="", contexto="", narrativa="", menciones=None, max_browsers=None, permitir_pausadas=False, duracion_max_min=120, cancelar=None, callback=None)` — 100% aditivo: resuelve cuentas (pausadas fuera salvo `permitir_pausadas=True`; Tier 2/3 omitidas por el blindaje de hashtags; cuotas horarias/diarias reservadas por post), genera N textos por cuenta, publica los N tweets CONSECUTIVOS por cuenta con pausa aleatoria de 60-240s entre ellos (0.5-2s entre cuentas), worker-pool acotado (default `MAX_BROWSERS`, tope 3), tope `duracion_max_min`, paro total (`cancelar`), URLs capturadas por post y resumen con `por_cuenta`/`total_esperado`/`omitidas_por_tiempo`/`omitidas_por_cuota`/`pausadas_*`/`tier_*`. Reutiliza la maquinaria de acciones/cuotas/tiers/pestaña persistente/sesión caída. `tests/test_actividad_motor.py` 63 checks.
- `ia/generador_contenido.py`: `generar_textos_actividad_por_cuenta(cuentas_info, hashtags, n_por_cuenta, contexto="", narrativa="", menciones=None) -> dict[usuario, list[textos]]` + `verificar_cobertura_actividad(textos, hashtags)`: **TODOS los hashtags en cada texto si caben en 100 chars**; si no caben, reparto con cobertura total por cuenta (nunca descarta tags); textos únicos, respetan registro/perfil/personalidad, hashtag en medio, narrativa invisible, menciones al final si caben; fallback local por registro. `ia/prompts.py`: `get_prompt_hashtags(..., todos=True)`. `tests/test_actividad_textos.py` 69 checks + fuzz 50 (0 violaciones).
- `web/operaciones/actividad.py` (NUEVO) + `web/app.py`: operación **"✍️ Actividad"** (categoría ⚡ Automatización Twitter) con selector de cuentas (reutiliza `_selector_masivo`), hashtags 1-6 obligatorios, posts 3-4, pausas 60-240s, navegadores 1-3 (default 2), duración máx 120 min, contexto + panel de noticias, menciones, checkbox "incluir cuentas pausadas (mantenimiento leve)" y panel de progreso/paro total/JSON de campaña reutilizados de `activacion_masiva.py` (`tipo="actividad"`). `tests/test_actividad_web.py` 28 checks.
- Verificado: compileall global OK; suite **2386/2386**; smoke del coordinador 50/50 con IA caída (4 hashtags en cada uno de los 10 textos, ≤100, sin hashtag final, sin ajenos, únicos y con el registro de cada cuenta).

### Importador inmune al orden de columnas: heurística por forma (2026-09-24)
- **Pedido**: no reacomodar el Excel a mano por cada lote de proveedor nuevo; que el sistema detecte los datos por su FORMA sin importar el orden de las columnas.
- `cuentas/importador.py`: tras la extracción de JSON de cookies / User-Agent y del `split(':')`, nueva reasignación heurística (`_detectar_tipo_campo` + `_reasignar_campos_por_forma`, ~L372-471): **auth_token** = 40 hex minúscula (`^[a-f0-9]{40}$`), **totp_secret** = 16 base32 mayúsculas (`^[A-Z2-7]{16}$`), **email** = `^[^@\s]+@[^@\s]+\.[A-Za-z]{2,}$`; funciona en CUALQUIER índice. Reglas de conflicto: prioridad al campo que ya está en su posición canónica (una password con forma de token NO pisa un auth_token real), un solo reclamo por variable y los campos no detectados se asignan posicionalmente a las variables libres en el orden estándar. El formato vendedor de 7 campos conserva `permitir_auth=False` (cookies `[ct0, auth_token]` intactas). Contrato de claves de `parsear_linea` e `importar_una` sin cambios; malformadas (>8 campos) igual que antes.
- Verificado: `compileall` OK; suite **2404/2404** (`tests/test_importador.py` 140/140, +18 checks de desorden/conflictos); smoke del coordinador 52/52 (clásico, email↔totp intercambiados, token en medio, email primero, conflicto de tokens, vendedor 7 campos, sin token, basura y malformada).

### Railway: arranque de bots (supervisord) + arreglar_columnas.py + bot clientes solo TOTP (2026-09-24)
- **Bug de produccion (bots caidos en Railway)**: el `railway up` mostraba `spawned: 'xvfb'/'dashboard'/'scheduler'` y NINGUN bot. Causa: `supervisord.conf` solo definia esos 3 programas. Fix: nuevos `[program:bot]` (`python -m bot.main`) y `[program:bot-clientes]` (`python -m bot_clientes.main`) con `startsecs=15`/`startretries=5` (si falta el token el proceso sale con codigo 1 y queda FATAL, sin loop infinito); `docker-compose.yml` sin el servicio `bot-clientes` (evita dos procesos con el mismo token). Tras el redeploy el log debe mostrar `spawned: 'bot'` y `spawned: 'bot-clientes'`.
- **`arreglar_columnas.py` (nuevo, raiz)**: repara cuentas cuyo lote llego con las COLUMNAS CORRIDAS (`auth_token` = 16 mayusculas = TOTP real; `totp_secret` con `@` = email real; `email_password` = token hex de 40). Intercambia exactamente esos 4 campos (`totp_secret`←auth_token, `auth_token`←email_password, `email`←totp_secret, `email_password`="") con un `db.commit()` por cuenta, SIN tocar tiers/secciones/fotos/roles/status; `--dry-run`/`--json`; usa `get_db_session()` de core.database (no existe `obtener_sesion`). `tests/test_arreglar_columnas.py` 45 checks; smoke del coordinador 15/15 (dry-run no escribe, apply repara, intactos, idempotente).
- **Bot de clientes SOLO TOTP**: se elimino la via de correo (`codigo_desde_correo`, import del lector, reintentos): el flujo lee `Cuenta.totp_secret` y genera el codigo con pyotp; sin semilla muestra un mensaje amable ("pide ayuda a quien te dio la cuenta"). `utils/lector_correo.py` queda como modulo sin uso por el bot. `tests/test_bot_clientes.py` 120/120.
- Verificado: `compileall` global OK; suite **2477/2477**; prueba independiente del coordinador del script 15/15.

### Ordenes de compra: 60 cuentas importadas + fix del parser de 5 campos (2026-09-24)
- **Ordenes**: `order8830212.txt` (15 cuentas, 5 campos `usuario:password:email:email_password:auth_token40`, SIN TOTP — incluye las 5 clientes "con seguidores") y `order8830346.txt` (45 cuentas, 6 campos con `:totp16` al final — incluye las 10 clientes "sin seguidores"). Copiadas a `data/clientes/` (ignorado en git).
- **Bug del parser (detectado por el coordinador)**: la heuristica metia el `email_password` de una linea de 5 campos SIN TOTP dentro de `totp_secret` (y dejaba `email_password` vacio) en las 15/15 lineas. Fix en `cuentas/importador.py`: los campos NO detectados ahora llenan los huecos del orden canonico **POR SEGMENTOS** entre campos detectados (`_reasignar_campos_por_forma`), con fallback posicional historico para casos ambiguos; `tests/test_importador.py` 149/149.
- **`importar_ordenes.py` (nuevo, raiz)**: CLI `python importar_ordenes.py [archivo...] [--apply] [--json] [--pausar "u1,u2"] [--pausar-archivo RUTA]` (default: `data/clientes/order*.txt` y pausa a los usuarios de `data/clientes/a_importar.txt`); upsert por usuario actualizando SOLO password/email/email_password/auth_token/totp_secret NO vacios; nunca toca cookies/seccion/tier/avatares/rol/status; dry-run por defecto. `tests/test_importar_ordenes.py` 44 checks.
- **Import local ejecutado**: 60 lineas → **45 creadas + 15 actualizadas, 0 errores**; las 10 clientes con TOTP validado con pyotp (10/10 generan codigo de 6), las 5 sin TOTP quedan con semilla vacia y `email_password` correcto, 15/15 siguen pausadas. BD local: 92 cuentas twitter.
- ⚠ Las 5 cuentas cliente "con seguidores" NO traen semilla TOTP en su orden: el bot de clientes dira "no tiene el codigo 2FA configurado" hasta obtener/configurar su semilla.
- **Aplicado en Supabase (2026-09-24)**: `importar_ordenes.py --apply` corrido por el coordinador (variables de Railway) → **60/60 actualizadas, 0 errores**; se limpio la basura desplazada de `totp_secret` en las 15 cuentas de la orden 1 (no traen semilla); verificacion final en Supabase: **45 TOTP validas + 15 vacias + 0 invalidas**; clientes: **10 con TOTP valido, 5 sin semilla, 15/15 pausadas**. BD: 260 cuentas twitter.
- Verificado: `compileall` global OK; suite **2530/2530**; smoke del coordinador 17/17 (fix del parser casos A/B, 15/15 en BD, 10/10 TOTP funcional, 5/5 sin semilla).

### Bot de clientes: modo grupo único con las 15 cuentas globales (2026-09-24)
- **Cambio de modelo pedido por el dueño** (el reparto por Telegram ID era tardado): el bot de clientes (@vrf2fa_bot) funciona SOLO en el grupo `-1005538610567` y maneja EXACTAMENTE las 15 cuentas de cliente como lista GLOBAL del grupo (cualquier miembro las usa con botones; sin registro por usuario).
- `bot_clientes/clientes_store.py` reescrito: estructura `{"chat_id": -1005538610567, "cuentas": [las 15]}` con migración automática del formato viejo (une cuentas únicas, descarta Telegram IDs y reescribe el archivo); `chat_id()` (env `TELEGRAM_CLIENTES_CHAT_ID` → JSON → default), `cuentas()`, `es_chat_permitido()`; siembra los defaults si el archivo no existe.
- `bot_clientes/handlers.py`: guard por CHAT (privado → "ℹ️ Este bot funciona únicamente en el grupo de clientes."; otro grupo → silencio; el grupo permitido → todo); eliminados el guard por usuario y `/asignar`/`/quitar`/`/clientes`; menú/lista con las 15 y su nombre registrado; nuevo comando admin **`/nombre <usuario> <Nuevo Nombre>`** (actualiza `Cuenta.nombre_mostrado` en la BD, sin Chrome; cambiar EN X sigue siendo el botón ✏️). `.env.example`/`.env` con `TELEGRAM_CLIENTES_CHAT_ID=-1005538610567`; README actualizado.
- Verificado: `compileall` global OK; suite **2512/2512** (los checks del modelo viejo se reemplazaron por los del nuevo); smoke del coordinador 10/10 + comandos registrados `start/ayuda/help/cancelar/nombre` (sin `/asignar`).

### Bot de clientes: matriz de acceso (grupo = solo 2FA / privado = solo admin) (2026-09-24)
- **Pedido del dueño**: en el GRUPO cualquiera puede usar el bot pero SOLO la opción de código 2FA; en PRIVADO funciona solo para el dueño (admin) y con TODAS las opciones, para verificar el funcionamiento. El «no aparece nada» del grupo era que el bot aún no era miembro.
- `bot_clientes/handlers.py`: `_contexto(update)` → `"grupo"` (chat permitido) / `"admin_privado"` (privado + `TELEGRAM_ADMIN_IDS`) / `"bloqueado"`; menú **reducido** en grupo (🔑 + ❓) y **completo** en privado-admin; privado ajeno → "ℹ️ Por privado solo funciona para el administrador…"; otros grupos → silencio. Defensa: callbacks/flujos de nombre/fotos y `/nombre` se rechazan fuera de `admin_privado` (botones viejos del grupo incluidos).
...
- **Redeploy en Railway** para aplicar el paro total y el panel de proceso persistente; probar: lanzar 5-10 cuentas, refrescar/navegar a otra operacion y volver (el proceso debe seguir visible), `⛔ Paro total` (el guard se libera en segundos) y relanzar de inmediato
- **Redeploy en Railway** para aplicar Tiers, Curva de Aceleración y cuota diaria (`LIMITE_DIARIO_POR_CUENTA=12`, `PESTANA_MAX_MINUTOS=20`) + el blindaje del compositor y la validación de hashtags; asignar Tier 1/Tier 2 a la flota (pestaña «🏅 Tiers») y probar una campaña de curva con 5-10 cuentas (fase 1 solo Tier 1, fase 2 en cascada) midiendo acciones/min y RAM
...
- Asignar seccion (IP/CI/Libertad/Justicia) y tipo (politica/activista/ciudadana) a las 200 cuentas de la flota que quedaron en Supabase (hoy todas activas) y validar sus sesiones (Cuentas -> Validar) antes de calentarlas; respaldo completo previo en `data/backups/limpieza_cuentas_20260921_142129.json` y herramienta `restaurar_cuentas.py`; las 25 cuentas vendedoras se eliminaron con la nueva pestana/CLI (respaldo `data/backups/eliminacion_cuentas_20260922_112839.json`)
- **Redeploy en Railway** para que el dashboard tenga la pestana "🗑️ Eliminar" (el CLI `eliminar_cuentas.py` corre local con `DATABASE_URL` exportada, no necesita deploy)
- **Redeploy en Railway (bot)** para que exista `/codigo`; agregar los Telegram IDs del equipo a `TELEGRAM_ADMIN_IDS` y probar en el grupo con una cuenta que ya tenga `totp_secret`
- **Bot de clientes**: agregar **@vrf2fa_bot** al grupo `-1005538610567` (aún NO es miembro — el «no aparece nada» era eso; verificado con `getChat`=400); en el grupo solo se usa `/start` + botones (ya NO hace falta Group Privacy); en el **privado del admin** está el menú completo para pruebas; `TELEGRAM_CLIENTES_BOT_TOKEN` y `TELEGRAM_CLIENTES_CHAT_ID` en las variables del servicio de Railway
- **Supabase al día (2026-09-24, hecho por el coordinador)**: las 60 cuentas quedaron con sus datos correctos (`importar_ordenes.py --apply`: 60/60, 0 errores) y las semillas-basura se limpiaron; `arreglar_columnas.py` e `importar_ordenes.py` quedan como herramientas para futuros lotes con columnas corridas
- **Redeploy en Railway**: ahora `supervisord` arranca los 2 bots — verificar en el log `spawned: 'bot'` y `spawned: 'bot-clientes'` y que `TELEGRAM_BOT_TOKEN` (@enigmawebi_bot) y `TELEGRAM_CLIENTES_BOT_TOKEN` (@vrf2fa_bot) estén en las variables del servicio, DISTINTOS entre sí
- **App passwords de Yahoo (DESCARTADO para el bot)**: el bot de clientes ya NO usa el correo (solo TOTP); queda solo como referencia histórica: Yahoo rechazó la password normal por IMAP en las 15 cuentas
- Importar las 15 cuentas de cliente a la BD de Railway/Supabase: exportar `DATABASE_URL` y correr `python importar_clientes.py --apply` (local ya están, 15/15 pausadas)
- Confirmar la sección (CI/Libertad) de cada grupo de cuentas de cliente (las 5 con seguidores y las 10 sin seguidores): asignarla en «🗂️ Cuentas» y luego pausarlas ya está hecho; el bot de clientes no depende de la sección
- Para que el bot de clientes cambie nombre/fotos: las 15 cuentas NO tienen cookies/sesión en la BD; importar cookies tras un login válido o decidir si se implementa login automático del servidor (password + código por correo) cuando las app passwords de Yahoo funcionen
- **Redeploy en Railway** para aplicar el fix de la curva Tier 1 (anticipación de fase 2 + `curva_fase2_anticipada`/`curva_fase2_motivo` en el JSON de campaña) y las claves nuevas de pausadas (`pausadas_omitidas`/`pausadas_usuarios`)
- **Redeploy en Railway** para que el dashboard tenga la operación "✍️ Actividad" (modo leve: 3-4 tweets por cuenta con TODOS los hashtags); probar con 2-3 cuentas Tier 1 y hashtags reales (ajustar pausas/navegadores según se quiera más leve)
- **Redeploy en Railway (dashboard)** para que la pestaña Importar use la heurística por forma (el `cuentas/importador.py` local ya es inmune al orden de columnas; el CLI/API local no necesita deploy)
