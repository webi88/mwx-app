...
- **Correr `arreglar_columnas.py` contra Supabase** (`DATABASE_URL` exportada) para recuperar las semillas TOTP que quedaron en `auth_token` y los tokens en `email_password`: sin eso el bot de clientes dirá "esta cuenta no tiene el código 2FA configurado" (recomendado: primero `--dry-run`)
- **Correr `importar_ordenes.py --apply` contra Supabase** (`DATABASE_URL` exportada) con las órdenes de `data/clientes/order*.txt`: sube las 60 cuentas con sus semillas TOTP correctas (las 10 clientes del archivo 2 quedan con TOTP funcional; las 5 "con seguidores" no traen semilla en su orden). Alternativa/complemento: `arreglar_columnas.py` si las cuentas ya están en Supabase pero desplazadas
