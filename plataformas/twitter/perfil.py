...
2. Leer el perfil real de una cuenta usando sus cookies guardadas
   (`auth_token` + `ct0`) por HTTP (`obtener_perfil_http`), siguiendo el patron
   de `plataformas/twitter/session_validator.py`.
...
def obtener_perfil_http(
    ...
    - GET https://x.com/{usuario} con `follow_redirects=True`.
    - Con `auth_token` usa los headers de `session_validator.construir_headers`;
      sin sesion usa headers basicos de Chrome.
    ...
    try:
        # Sin ct0 la API/pagina autenticada de X no responde: se pide uno fresco.
        ...
        if resp.status_code >= 400:
            logger.info(f"Perfil de {usuario}: HTTP {resp.status_code}")
            return None
        ...
        for frase in _FRASES_BLOQUEO:
            if frase in bajo:
                logger.warning(
                    f"Perfil de {usuario}: pagina bloqueada/suspendida ('{frase}')"
                )
                return None

        logger.info(f"Perfil de {usuario}: sin datos parseables en el HTML")
        return None
...
def sincronizar_cuenta(cuenta, timeout: int = 20) -> dict:
    ...
    `cuenta` es un objeto `core.models.Cuenta`. Devuelve:
    {"usuario", "nombre", "handle", "cambio_nombre", "cambio_handle", "error"}
    donde `error` es "" si todo ok, "sin_datos" si no se pudo leer el perfil,
    o el detalle del fallo. NUNCA lanza excepcion.
    """
    resultado = {
        "usuario": "",
        "nombre": "",
